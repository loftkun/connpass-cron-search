...
# count all evnets
# mongo --eval "db.${DB}.find().count();" -u ${USER} -p ${PASS} --host ${HOST} --quiet

# search all event_id
# mongo --eval "db.${DB}.find().forEach(function(e){ print(e.event_id); });" -u ${USER} -p ${PASS} --host ${HOST} --quiet

# search one event
# mongo --eval "db.${DB}.find({event_id:135117});" -u ${USER} -p ${PASS} --host ${HOST} --quiet

# drop collection
# mongo --eval "db.${DB}.drop();" -u ${USER} -p ${PASS} --host ${HOST} --quiet


exec() {
    echo "exec > ${1}"
    sudo docker exec -it ${CONTAINER} sh -c "echo \"${1}\" | mongo -u ${USER} -p ${PASS} --host ${HOST} --quiet"
}

exec "show dbs"
exec "show collections"

# sudo docker exec -it ${CONTAINER} mongo -u ${USER} -p ${PASS} --host ${HOST} --quiet
# > db.connpass.drop()
# > db.connpass.find()
